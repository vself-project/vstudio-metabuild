...
/// Constrain a linear combination to be equal to a variable
pub fn constrain_lc_with_variable<CS: ConstraintSystem>(
    cs: &mut CS,
    lc: LinearCombination,
    variable: &Variable,
) {
    cs.constrain(lc - LinearCombination::from(*variable));
}
